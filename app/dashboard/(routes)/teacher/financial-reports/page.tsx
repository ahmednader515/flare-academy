            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        View financial summaries and payment details for each course
                    </p>
                </div>
            </div>
                ...
                <CardContent>
                    <Select value={selectedCourseId} onValueChange={setSelectedCourseId}>
                        <SelectTrigger className="w-full max-w-md">
                            <SelectValue placeholder="Select a course to view financial report" />
                        </SelectTrigger>
                        <SelectContent>
                            {courses.map((course) => (
                                <SelectItem key={course.id} value={course.id}>
                                    {course.title} - {course.price || 0} EGP
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
            ...
            {report && (
                <>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-green-600">
                                    {report.summary.totalPaid.toFixed(2)} EGP
                                </div>
                            </CardContent>
                        </Card>
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-red-600">
                                    {report.summary.totalRemaining.toFixed(2)} EGP
                                </div>
                            </CardContent>
                        </Card>
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">
                                    {report.summary.totalExpected.toFixed(2)} EGP
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <CardTitle>Student Payments - {report.course.title}</CardTitle>
                                        ...
                                        {loadingReport ? (
                                            ...
                                            filteredPayments.map((sp, index) => (
                                                <TableRow key={sp.studentId}>
                                                    ...
                                                    <TableCell className="font-medium">{sp.studentName}</TableCell>
                                                    <TableCell>{sp.studentPhone}</TableCell>
                                                    <TableCell>{sp.studentCollege || "N/A"}</TableCell>
                                                    <TableCell>{sp.studentFaculty || "N/A"}</TableCell>
                                                    <TableCell>{sp.coursePrice.toFixed(2)} EGP</TableCell>
                                                    <TableCell>{sp.totalPaid.toFixed(2)} EGP</TableCell>
                                                    <TableCell>
                                                        <span className={sp.remaining > 0 ? "text-red-600 font-semibold" : "text-green-600"}>
                                                            {sp.remaining.toFixed(2)} EGP
                                                        </span>
                                                    </TableCell>
                                                    <TableCell>
                                                        <Badge variant={sp.isFullyPaid ? "default" : "secondary"}>
                                                            {sp.isFullyPaid ? "Fully Paid" : "Pending"}
                                                        </Badge>
                                                    </TableCell>
                                                    <TableCell>
                                                        <Badge variant="outline">
                                                            {sp.payments.length} payment{sp.payments.length !== 1 ? 's' : ''}
                                                        </Badge>
                                                    </TableCell>
                                                </TableRow>
                                            ))
